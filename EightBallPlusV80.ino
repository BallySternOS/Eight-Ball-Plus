/**************************************************************************
  ...
  Ver 75 - First release of full Eight Ball Plus game code
  Ver 76 - Addition of extended Bank Shot functions
  Ver 77 - Goals display at ball start
  Ver 78 - OverRide Scores priority
  Ver 79 - Fix flashing arrows from being overwritten
  Ver 80 - 15 Ball mode 8 ball was awarding multiple times.  Chase Ball countdown was not resetting.
...
//
  ...
  if ( (NextBall != 0) && ((CurrentTime - NextBallTime) < (NextBallDuration*1000)) ) {
    ...
    #endif
    if (!(GameMode[CurrentPlayer] == 3)) {
      BSOS_SetLampState((LA_SMALL_1 + (NextBall-1) + (CurrentPlayer%2 ? 8:0)), 1);  // Revert to steady on
    }
    NextBall = 0;
    NextBallTime = 0;
    //BallLighting();
...
void BonusMultiplier(byte mult){
  switch (mult){
    ...
    case 5: // 5X
      ...
      break;
    case 7: // 7X
      BSOS_SetLampState(LA_2X, 1, 0, 1000);
      BSOS_SetLampState(LA_3X, 0);
      BSOS_SetLampState(LA_5X, 1, 0, 1000);
      BonusMult = 7;
      break;
    case 8: // 8X
      BSOS_SetLampState(LA_2X, 0);
      BSOS_SetLampState(LA_3X, 1, 0, 750);
      BSOS_SetLampState(LA_5X, 1, 0, 750);
      BonusMult = 8;
      break;
    case 10: // 10X
      BSOS_SetLampState(LA_2X, 1, 0, 400);
      BSOS_SetLampState(LA_3X, 1, 0, 500);
      BSOS_SetLampState(LA_5X, 1, 0, 600);
      BonusMult = 10;
      break;
...
void BankShotLighting(){
  ...
  if (BankShotProgress > 6) {
    switch (BankShotProgress) {
      ...
      case 10:
      case 13:
        BSOS_SetLampState(LA_BANK_SHOT_5000, 1, 0, 600);
        break;
      case 8:
      case 11:
      case 14:
        BSOS_SetLampState(LA_BANK_SHOT_5000, 1, 0, 300);
        break;
      case 15:
        BSOS_SetLampState(LA_BANK_SHOT_5000, 1, 0, 100);
        break;
      default:
        BSOS_SetLampState(LA_BANK_SHOT_5000, 1);
        break;
    }
  }
}
