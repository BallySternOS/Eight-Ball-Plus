/**************************************************************************
    Eight Ball Plus is distributed in the hope that it will be useful,
...
#include "EightBallPlus.h"
#include "SelfTestAndAudit.h"
#include "BSOS_Config.h"
#include <EEPROM.h>

#define VERSION_NUMBER    75
...
#define MACHINE_STATE_ADJUST_CHASEBALL_DURATION         -19
#define MACHINE_STATE_ADJUST_SPINNER_COMBO_DURATION     -20
#define MACHINE_STATE_ADJUST_SPINNER_THRESHOLD          -21
#define MACHINE_STATE_ADJUST_POP_THRESHOLD              -22
#define MACHINE_STATE_ADJUST_NEXT_BALL_DURATION         -23
#define MACHINE_STATE_ADJUST_TILT_WARNING               -24
#define MACHINE_STATE_ADJUST_BALLS_OVERRIDE             -25
#define MACHINE_STATE_ADJUST_DONE                       -26
...
#define CHASEBALL_DURATION_MAX    26
#define CHASEBALL_DURATION_DEF    20              // Length of each segment
...
//  for (int byteNum = 0; byteNum < 8; byteNum++) {
    for (byte bitNum = 0; bitNum < 8; bitNum++) {
      ...
      if (subOffset) {
        ...
        if (lampOff && lampNum != keepLampOn) BSOS_SetLampState((lampNum + ((AuxLamps)?60:0) ), 0);
//        if (lampOff && lampNum != keepLampOn) BSOS_SetLampState(lampNum, 0);
      }

      byte lampOn = false;
      lampOn = animationNum[currentStep][byteNum] & (1 << bitNum);
      if (lampOn) BSOS_SetLampState((lampNum + ((AuxLamps)?60:0)), 1, dim);
...
void SetPlayerLamps(byte numPlayers, byte playerOffset = 0, int flashPeriod = 0) {
  ...
  for (int count = 0; count < 4; count++) {
    BSOS_SetLampState(LA_1_PLAYER + playerOffset + count, (numPlayers == (count + 1)) ? 1 : 0, 0, flashPeriod);
  }
}
...
int InitNewBall(bool curStateChanged, byte playerNum, int ballNum) {  
  ...
  if (curStateChanged) {
    ...
    ArrowTest = true;
    BSOS_SetLampState(LA_LEFT_ARROW, 0);  // Reset arrows
    BSOS_SetLampState(LA_RIGHT_ARROW, 0);  // Reset arrows 
    ...
    BSOS_SetLampState(LA_SPINNER, 0);       // Spinner and Kicker are off at beginning of a new ball
...
//
    ...
    if (OutlaneSpecial[CurrentPlayer] == false) {
      BSOS_SetLampState(LA_OUTLANE_SPECIAL, 0);                    // Turn off lamp
    } else {
      BSOS_SetLampState(LA_OUTLANE_SPECIAL, 1);                    // If current player is true, turn on lamp
    }
    if (GameMode[CurrentPlayer] == 1) {
      BSOS_SetLampState(LA_SUPER_BONUS, 0);                        // Turn off SuperBonus lamp
      if (Balls[CurrentPlayer] & (0b1<<15)) {                      // If player already has SuperBonus
        BSOS_SetLampState(LA_SUPER_BONUS, 1);                      // Turn on SuperBonus lamp
      }
      if ((Balls[CurrentPlayer] & (0b1<<7)) == 128) {              // If 8 ball collected, we have achieved SuperBonus
        ...
        if (!OutlaneSpecial[CurrentPlayer]) {                      // Achieving SuperBonus turns on Outlane Special
          ...
          BSOS_SetLampState(LA_OUTLANE_SPECIAL, 1);                // Turn lamp on, signifying mode is active
        }
...
//
    ...
    if (GameMode[CurrentPlayer] == 2) {                  // if 15 Ball mode already active
      ...
      BSOS_SetLampState(LA_SUPER_BONUS, 1, 0, 750);     // Turn on slow blink for Mode 2
    }
    if (FifteenBallCounter[CurrentPlayer] > 2) {        // Maximum balls in Mode 2
      ...
      BSOS_SetLampState(LA_SUPER_BONUS, 0);             // Turn off
    }
    ...
    BSOS_SetLampState(LA_SPSA_BACK, 0);        
    ...
    if (curStateChanged) {
      ...
      switch (curState) {
        ...
        case MACHINE_STATE_ADJUST_CHASEBALL_DURATION:
          ...
          AdjustmentValues[1] = 17;
          AdjustmentValues[2] = 20;
          AdjustmentValues[3] = 23;
          AdjustmentValues[4] = 26;
          AdjustmentValues[5] = 99;
  ...
  for (byte scoreCount = 0; scoreCount < 4; scoreCount++) {   // Loop on scores
    ...
    if (allScoresShowValue == 0 && (ScoreOverrideStatus & (0x10 << scoreCount))) {
      ...
      if (displayToUpdate == 0xFF || displayToUpdate == scoreCount || displayScore > BALLY_STERN_OS_MAX_DISPLAY_SCORE || showingCurrentAchievement) {
        ...
        if (displayScore > BALLY_STERN_OS_MAX_DISPLAY_SCORE) {    //  Need to scroll
          ...
          if ((CurrentTime - LastTimeScoreChanged) < ((MachineState == 0)?1000:4000) ) {  // How long to wait to before 1st scroll
...
#endif            
            BSOS_SetDisplayBlank(scoreCount, blank);            // Sets all digits on except singles for blinking
            ...
            if (scrollPhase < 11 && scrollPhaseChanged) {   // Scroll phase 0-10 is for actual scrolling
              ...
              if (scrollPhase < BALLY_STERN_OS_NUM_DIGITS) {    // Scrolling lowest 6 digits or less
                displayMask = BALLY_STERN_OS_ALL_DIGITS_MASK;
                for (byte scrollCount = 0; scrollCount < scrollPhase; scrollCount++) {  
                  displayScore = (displayScore % (BALLY_STERN_OS_MAX_DISPLAY_SCORE + 1)) * 10;  // *10 shift for each scrolled digit up to scrollPhase
...
#if 0
  BSOS_WriteByteToEEProm(BSOS_CREDITS_EEPROM_BYTE, 5);
  BSOS_WriteULToEEProm(BSOS_TOTAL_PLAYS_EEPROM_START_BYTE, 0);
  BSOS_WriteULToEEProm(BSOS_HIGHSCORE_EEPROM_START_BYTE, 101000);
  BSOS_WriteULToEEProm(BSOS_AWARD_SCORE_1_EEPROM_START_BYTE, 270000);
  BSOS_WriteULToEEProm(BSOS_AWARD_SCORE_2_EEPROM_START_BYTE, 430000);
  BSOS_WriteULToEEProm(BSOS_AWARD_SCORE_3_EEPROM_START_BYTE, 650000);
  BSOS_WriteULToEEProm(BSOS_TOTAL_REPLAYS_EEPROM_START_BYTE, 0);
  BSOS_WriteULToEEProm(BSOS_TOTAL_HISCORE_BEATEN_START_BYTE, 0);
  BSOS_WriteULToEEProm(BSOS_CHUTE_1_COINS_START_BYTE, 0);
  BSOS_WriteULToEEProm(BSOS_CHUTE_2_COINS_START_BYTE, 0);
  BSOS_WriteULToEEProm(BSOS_CHUTE_3_COINS_START_BYTE, 0);
#endif
    ...
    Serial.println(BSOS_ReadByteFromEEProm(BSOS_CREDITS_EEPROM_BYTE), DEC);
    Serial.print(F("AwardScores[0]:          "));
    Serial.println(BSOS_ReadULFromEEProm(BSOS_AWARD_SCORE_1_EEPROM_START_BYTE), DEC);
    Serial.print(F("AwardScores[1]:          "));
    Serial.println(BSOS_ReadULFromEEProm(BSOS_AWARD_SCORE_2_EEPROM_START_BYTE), DEC);
    Serial.print(F("AwardScores[2]:          "));
    Serial.println(BSOS_ReadULFromEEProm(BSOS_AWARD_SCORE_3_EEPROM_START_BYTE), DEC);
    Serial.print(F("High Score:              "));
    Serial.println(BSOS_ReadULFromEEProm(BSOS_HIGHSCORE_EEPROM_START_BYTE, 10000), DEC);
...
//
    ...
    //
      ...
      if ((CurrentTime-AttractStepTime)>RackDelayLength) { // global variable msec delay between steps
        ...
        BSOS_SetLampState((RACK_BALL_START+AttractStepLights), 1, 0, 0);
        AttractStepLights +=1;
      }
...
//      
    if (( (CurrentTime-AttractOffset+6750)/10000)%2==0) {
      if (AttractPlayfieldMode!=2) {
        ...
        BSOS_TurnOffAttractLamps(); // Start mode by blanking all lamps
...
//  Lamp animations by frames
      ...
      if ((CurrentTime-AttractSweepTime)>30) { // Value in msec delay between animation steps
        ...
        for (byte lightcountdown=0; lightcountdown<NUM_OF_TRIANGLE_LAMPS_CW; lightcountdown++) {
          ...
          BSOS_SetLampState(AttractLampsDown[lightcountdown].lightNumDown, (dist<8), (dist==0)?0:dist/3, (dist>5)?(100+AttractLampsDown[lightcountdown].lightNumDown):0);
          if (lightcountdown==(NUM_OF_TRIANGLE_LAMPS_CW/2)) BSOS_DataRead(0);
        }
...
int NormalGamePlay() {
  ...
  if (BallFirstSwitchHitTime==0) {
    ...
    // Added this line to keep score updated.  Change out for Meteor code at some point.
    //BSOS_SetDisplay(CurrentPlayer, CurrentScores[CurrentPlayer], true);
  }
  //ShowPlayerScores(CurrentPlayer, (BallFirstSwitchHitTime == 0) ? true:false, true);
...
//    Serial.print(F("                    "));
//    Serial.println(Balls[1], BIN);  
//    Serial.print(F("                    "));
//    Serial.println(Balls[2], BIN);  
//    Serial.print(F("                    "));
//    Serial.println(Balls[3], BIN);  
...
//    Serial.print(F("Ball8Check is:                 "));
//    Serial.println(Ball8Check, DEC);  
//    Serial.print(F("Toggle8Lamps is:               "));
//    Serial.println(Toggle8Lamps, DEC);  
...
/*
    Serial.print(F("Sample #define:           "));
    Serial.println(BSOS_CREDITS_EEPROM_BYTE, DEC);  
    Serial.println(NONSENSE_DEFINE, DEC);
    Serial.println(BSOS_HIGHSCORE_EEPROM_START_BYTE, DEC);  
    Serial.println(BSOS_AWARD_SCORE_1_EEPROM_START_BYTE, DEC);  
    Serial.println(BSOS_AWARD_SCORE_2_EEPROM_START_BYTE, DEC);  
    Serial.println(BSOS_AWARD_SCORE_3_EEPROM_START_BYTE, DEC);
    Serial.println(BSOS_TOTAL_PLAYS_EEPROM_START_BYTE, DEC);
    Serial.println(BSOS_TOTAL_REPLAYS_EEPROM_START_BYTE, DEC);
    Serial.println(BSOS_TOTAL_HISCORE_BEATEN_START_BYTE, DEC);
    Serial.println(BSOS_CHUTE_2_COINS_START_BYTE, DEC);
    Serial.println(BSOS_CHUTE_1_COINS_START_BYTE, DEC);
    Serial.println(BSOS_CHUTE_3_COINS_START_BYTE, DEC);
    Serial.println();
*/
    Serial.println();
    Serial.println();  
...
//  if ( GameMode[CurrentPlayer] == 3  && ((CurrentTime - ChaseBallTimeStart) < CHASEBALL_DURATION) ) {
...
/*    if ((CurrentTime - ChaseBallTimeStart) < CHASEBALL_DURATION) {
    ...
    BSOS_SetLampState((LA_SMALL_1 - 1 + ChaseBall), 1, 0, LampBlinkTime);
...
//
  ...
  if ( (NextBall != 0) && ((CurrentTime - NextBallTime) < (NextBallDuration*1000)) ) {
    if (!(GameMode[CurrentPlayer] == 3)) {
      ...
      BSOS_SetLampState((LA_SMALL_1 + (NextBall-1) + (CurrentPlayer%2 ? 8:0)), 1, 0, 100);
      for (byte count = 0; count < 4; count++) {
        if (count != CurrentPlayer) {
          // Show time left
...
//
  ...
  if ( (((Balls[CurrentPlayer] & (0b01111111<<(CurrentPlayer%2 ? 8:0)))>>(CurrentPlayer%2 ? 8:0))==127) && (EightBallTest[CurrentPlayer]) ) {
    ...
    //Serial.println();
    BSOS_SetLampState(LA_SMALL_8, 1);
  }
...
// 
  ...
  if ( (SuperSpinnerAllowed[CurrentPlayer]) && ((CurrentTime - SuperSpinnerTime) < (SuperSpinnerDuration*1000)) ) {
    ...
    BallLighting();
    if (SpinnerKickerLit) {
      BSOS_SetLampState(LA_SPINNER, 1);        
    } else if ( (!SpinnerKickerLit) && (KickerOffTime != 0) ) {
      BSOS_SetLampState(LA_SPINNER, 1, 0, 100);
    } else {
      BSOS_SetLampState(LA_SPINNER, 0);
    }
  }
...
int InitGamePlay(boolean curStateChanged) {
  ...
  if (curStateChanged) {
    ...
    if (Credits > 0) {
      BSOS_SetLampState(LA_CREDIT_INDICATOR, 1);
    }
    ...
    for (int count=0; count<7; count++) {
      BSOS_SetLampState((LA_SMALL_1+count+8), 1);
    }
...
int CountdownBonus(boolean curStateChanged) {
  ...
  if (curStateChanged) {
    ...
    BSOS_SetLampState(LA_SPINNER, 0);
    if (GameMode[CurrentPlayer] == 2) BSOS_SetLampState(LA_SUPER_BONUS, 0);
    MaskCount = 0;
    ChimeLoop = 0;
    ...
    if (BonusBalls & (0b1<<MaskCount)) {                  // If true, ball has been captured
      if ( ChimeLoop < (3 * ((!Tilted)?BonusMult:1)) ) {  // Add bonus multiplier to total
        if ((CurrentTime - NextBonusUpdate) > BonusSpace) {
          if (!Tilted) {
            BSOS_PushToTimedSolenoidStack(SOL_CHIME_1000, 3, CurrentTime + 0, true);
            CurrentScores[CurrentPlayer] += 1000;
            if (GameMode[CurrentPlayer] == 2) {
              BSOS_PushToTimedSolenoidStack(SOL_CHIME_EXTRA, 3, CurrentTime + 0, true);
              CurrentScores[CurrentPlayer] += 1000;
            }
          }
          NextBonusUpdate = CurrentTime;
          ...
          BSOS_SetLampState((LA_BIG_1 + MaskCount), 0);                // Turn off lamp after chime sequence
          ChimeLoop = 0;
...
//    returnState = MACHINE_STATE_GAME_OVER;    
  ...
  byte switchHit;
    while ( (switchHit=BSOS_PullFirstFromSwitchStack())!=SWITCH_STACK_EMPTY ) {   // -A-
  ...
  //
        ...
        case SW_10_PTS:
          if (BallFirstSwitchHitTime == 0) BallFirstSwitchHitTime = CurrentTime;
          Ten_Pts_Stack += 1;
          //BSOS_PushToSolenoidStack(SOL_CHIME_10, 3);
          ...
          switch (GameMode[CurrentPlayer]) {
            case 1:
              if (((Balls[CurrentPlayer] & (0b11111111<<(CurrentPlayer%2 ? 7:0)))>>(CurrentPlayer%2 ? 7:0))==(CurrentPlayer%2 ? 254:127)) { // Check 8 bits, shift by 7 since 8 is common.
                ...
                BSOS_SetLampState(LA_SMALL_8, (MarqueeDisabled)?0:1, 0, 75);
                MarqueeTiming();    // Call multiple times for longer animmation length
                MarqueeTiming();
                MarqueeTiming();
                MarqueeTiming();          
                BSOS_PushToTimedSolenoidStack(SOL_KNOCKER, 3, CurrentTime + 900, true);
                ...
                break;
            case 2:
              if ((Balls[CurrentPlayer] & (0x7F7F))==(0x7F7F)) { // Check balls 1-7, 9-15.
                // Collect 8 Ball turn out small lamp
                BSOS_SetLampState(LA_SMALL_8, (MarqueeDisabled)?0:1, 0, 75);
                MarqueeTiming();    // Call multiple times for longer animmation length
                MarqueeTiming();
                ...
                BSOS_SetLampState(LA_SPSA_BACK, 1, 0, 175);
                BSOS_PushToTimedSolenoidStack(SOL_KNOCKER, 3, CurrentTime + 900, true);
                BSOS_PushToTimedSolenoidStack(SOL_KNOCKER, 3, CurrentTime + 1000, true);
                BSOS_PushToTimedSolenoidStack(SOL_KNOCKER, 3, CurrentTime + 1100, true);
                Balls[CurrentPlayer] = (Balls[CurrentPlayer] | 0b10000000);   // Raise flag for 8 Ball
                PlaySoundEffect(SOUND_EFFECT_8_BALL_CAPTURE);
        ...
        case SW_LEFT_OUTLANE:
          if (!KickerReady) {
            BSOS_PushToSolenoidStack(SOL_CHIME_1000, 3);
            CurrentScores[CurrentPlayer] += 1000;
          } else if ((KickerReady) && (!KickerUsed)) {
              SpinnerKickerLit = false;                   //Kicker turns off SpinnerKicker mode, leaves KickerReady running
              BSOS_SetLampState(LA_SPINNER, 1, 0, 100);   // Rapid blinking
              BSOS_PushToSolenoidStack(SOL_KICKER, 4);
        ...
        case SW_RIGHT_OUTLANE:
          ...
          BSOS_SetLampState(LA_OUTLANE_SPECIAL, 0);  // Turn lamp off after collecting Special
        ...
        case SW_RIGHT_SLING:
        case SW_LEFT_SLING:
          BSOS_PushToSolenoidStack(SOL_CHIME_100, 3);
          CurrentScores[CurrentPlayer] += 100;
          if (ArrowsLit[CurrentPlayer]) ArrowToggle();
          break;
        case SW_COIN_1:
        ...
        case SW_CREDIT_RESET:
          if (CurrentBallInPlay<2) {
            ...
            AddPlayer();
            if (Credits == 0) {
              BSOS_SetLampState(LA_CREDIT_INDICATOR, 0);
            }
          } else {
      ...
      for (int awardCount = 0; awardCount < 3; awardCount++) {
        if (AwardScores[awardCount] != 0 && scoreAtTop < AwardScores[awardCount] && CurrentScores[CurrentPlayer] >= AwardScores[awardCount]) {
          ...
          if (!SamePlayerShootsAgain) {
            ...
            BSOS_SetLampState(LA_SPSA_BACK, 1);        
...
void loop() {
  ...
  if (MachineState >= 0) {
    BSOS_SetDisplayCredits(CreditsDispValue, (CreditsFlashing)?((1-(CurrentTime/125)%2)):1, true);
    BSOS_SetDisplayBallInPlay(BIPDispValue, (BIPFlashing)?((CurrentTime/125)%2):1, true);
  }
...
void ShowShootAgainLamp() {
  ...
  if (!BallSaveUsed && BallSaveNumSeconds > 0 && (CurrentTime - BallFirstSwitchHitTime) < ((unsigned long)(BallSaveNumSeconds - 1) * 1000)) {
    ...
    BSOS_SetLampState(SAME_PLAYER, 1, 0, (msRemaining < 1000) ? 100 : 500);
  } else {
...
void SetGoals(byte goalnum) {   // Set goal flag and update display score
  ...
  if ( numOnes == 3 && !(Goals[CurrentPlayer] & (0b1<<6)) ) {       // Start Ball chase
    ...
    BSOS_PushToTimedSolenoidStack(SOL_KNOCKER, 3, CurrentTime + 750, true);
    BSOS_PushToTimedSolenoidStack(SOL_KNOCKER, 3, CurrentTime + 900, true);
    BSOS_PushToTimedSolenoidStack(SOL_KNOCKER, 3, CurrentTime + 1250, true);
    //MarqueeDisabled = true;
  ...
  if ( numOnes == 5 && !(Goals[CurrentPlayer] & (0b1<<7)) ) {       // Start 15 Ball mode
    ...
    BSOS_PushToTimedSolenoidStack(SOL_KNOCKER, 3, CurrentTime + 1050, true);
    BSOS_PushToTimedSolenoidStack(SOL_KNOCKER, 3, CurrentTime + 1350, true);
    BSOS_PushToTimedSolenoidStack(SOL_KNOCKER, 3, CurrentTime + 1450, true);
    BSOS_PushToTimedSolenoidStack(SOL_KNOCKER, 3, CurrentTime + 1550, true);
    //BSOS_DisableSolenoidStack();
    BSOS_SetDisableFlippers(true);
    MarqueeDisabled = true;
...
boolean CaptureBall(byte ballswitchnum) {
  ...
  if (GameMode[CurrentPlayer] == 1) {                  // Normal game play
    ...
    if (((Balls[CurrentPlayer] & (0b1<<((ballswitchnum-1) + (CurrentPlayer%2 ? 8:0))))>>((ballswitchnum-1) + (CurrentPlayer%2 ? 8:0))) == false) { // If flag is 0
      ...
      BSOS_SetLampState((LA_SMALL_1 + (ballswitchnum-1) + (CurrentPlayer%2 ? 8:0)), (MarqueeDisabled)?0:1, 0, 75);    // Set to rapid flashing upon capture
      MarqueeTiming();
  ...
  if ( (!(Balls[CurrentPlayer] & (0b1<<(ballswitchnum-1)))) && (!(Balls[CurrentPlayer] & (0b1<<((ballswitchnum-1) + (8))))) ) {
    ...
    BSOS_SetLampState((LA_SMALL_1 + (ballswitchnum-1) + 0), (MarqueeDisabled)?0:1, 0, 75);    // Set to rapid flashing upon capture
    MarqueeTiming();
    //BallLighting();
    ...
    BSOS_SetLampState((LA_SMALL_1 + (ballswitchnum-1) + 8), (MarqueeDisabled)?0:1, 0, 75);    // Set to rapid flashing upon capture
    MarqueeTiming();
    //BallLighting();
    Silent_Thousand_Pts_Stack +=5;                  // Award ball capture points
    PlaySoundEffect(SOUND_EFFECT_BALL_CAPTURE);
    ...
    BSOS_SetLampState((LA_SMALL_1 + (ballswitchnum-1) + 0), 1, 0, (MarqueeDisabled)?0:75); // Turn on
    MarqueeTiming();
    //BallLighting();
    ...
    BSOS_SetLampState((LA_SMALL_1 + (ballswitchnum-1) + 8), 1, 0, (MarqueeDisabled)?0:75); // Turn on
    MarqueeTiming();
    //BallLighting();
    Silent_Thousand_Pts_Stack -=10;                  // Award ball capture points
    //CurrentScores[CurrentPlayer] -=5000;          // Take away points
...
void ClearSmallBalls() {
  for(int i=0; i<15; i++) {                     // Turn out all small ball lamps
    //BSOS_SetLampState((LA_BIG_1 + i), 0);  
    BSOS_SetLampState((LA_SMALL_1 + i), 0);
  }
}
...
void FlashingArrows(int lamparray[], int rate, int numlamps) {
  for (int i = 0; i < numlamps; i++) {
    BSOS_SetLampState((lamparray[i]), 1, 0, rate);
    //BSOS_SetLampState((RackRightArrow[i]), 1, 0, rate);  
  }
}
...
void BonusMultiplier(byte mult){
  switch (mult){
    case 2: // 2X
      BSOS_SetLampState(LA_2X, 1);
      BSOS_SetLampState(LA_3X, 0);
      BSOS_SetLampState(LA_5X, 0);
      BonusMult = 2;
      break;
    case 3: // 3X
      BSOS_SetLampState(LA_2X, 0);
      BSOS_SetLampState(LA_3X, 1);
      BSOS_SetLampState(LA_5X, 0);
      BonusMult = 3;
      break;
    case 5: // 5X
...
void BankShotLighting(){
  ...
  #endif
  BSOS_SetLampState(LA_BANK_SHOT_300, ((BankShotProgress - 0) ? 0:1));
  BSOS_SetLampState(LA_BANK_SHOT_600, ((BankShotProgress - 1) ? 0:1));
  BSOS_SetLampState(LA_BANK_SHOT_900, ((BankShotProgress - 2) ? 0:1));
  BSOS_SetLampState(LA_BANK_SHOT_1200, ((BankShotProgress - 3) ? 0:1));
  BSOS_SetLampState(LA_BANK_SHOT_1500, ((BankShotProgress - 4) ? 0:1));
  BSOS_SetLampState(LA_BANK_SHOT_5000, ( ((BankShotProgress - 5) && (BankShotProgress - 6)) ? 0:1) );
}
...
void BankShotScoring(byte sound){
  ...
  BankShotProgress += 1;
  if (BankShotProgress>=7) BankShotProgress=6;
    switch (BankShotProgress) {
      ...
      case 5:
        ...
        //CurrentScores[CurrentPlayer] += 1500;
        // Light and set SPSA
        SamePlayerShootsAgain = true;
        BSOS_SetLampState(SAME_PLAYER, 1);
        BSOS_SetLampState(LA_SPSA_BACK, 1);        
...
void ArrowToggle() {
  if (RightArrow == false) {
    BSOS_SetLampState(LA_RIGHT_ARROW, 1);
    BSOS_SetLampState(LA_LEFT_ARROW, 0);
    RightArrow = true;
  } else {
    BSOS_SetLampState(LA_RIGHT_ARROW, 0);
    BSOS_SetLampState(LA_LEFT_ARROW, 1);
    RightArrow = false;    
  }
}
...
//  unsigned long count;  // not used?
  ...
  switch (soundEffectNum) {
    ...
    case SOUND_EFFECT_ADD_PLAYER:
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_10, 3, TimeStart, true);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_1000, 3, TimeStart, true);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_100, 3, TimeStart +200, true);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_1000, 3, TimeStart +400, true);
      break;
    case SOUND_EFFECT_BALL_OVER:
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_10, 3, TimeStart, true);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_10, 3, TimeStart+166, true);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_100, 3, TimeStart+250, true);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_100, 3, TimeStart+500, true);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_100, 3, TimeStart+750, true);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_10, 3, TimeStart+1000, true);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_100, 3, TimeStart+1166, true);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_1000, 3, TimeStart+1250, true);
      break;
    case SOUND_EFFECT_GAME_OVER: //  Ver 2
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_100, 3, TimeStart+0, true);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_100, 3, TimeStart+104, true);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_1000, 3, TimeStart+217, true);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_10, 3, TimeStart+404, true);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_10, 3, TimeStart+517, true);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_100, 3, TimeStart+629, true);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_EXTRA, 3, TimeStart+817, true);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_EXTRA, 3, TimeStart+925, true);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_10, 3, TimeStart+1042, true);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_10, 3, TimeStart+1229, true);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_10, 3, TimeStart+1342, true);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_10, 3, TimeStart+1458, true);
      break;
    case SOUND_EFFECT_MACHINE_START:
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_100, 3, TimeStart+217, true);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_100, 3, TimeStart+308, true);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_1000, 3, TimeStart+412, true);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_10, 3, TimeStart+579, true);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_10, 3, TimeStart+679, true);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_100, 3, TimeStart+779, true);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_EXTRA, 3, TimeStart+946, true);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_EXTRA, 3, TimeStart+1046, true);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_10, 3, TimeStart+1150, true);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_10, 3, TimeStart+1313, true);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_10, 3, TimeStart+1417, true);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_10, 3, TimeStart+1513, true);
      break;
    case SOUND_EFFECT_ADD_CREDIT:
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_EXTRA, 3, TimeStart, true);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_10, 3, TimeStart + 111, true);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_100, 3, TimeStart + 209, true);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_1000, 3, TimeStart + 310, true);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_100, 3, TimeStart + 477, true);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_1000, 3, TimeStart + 575, true);
      break;
    case SOUND_EFFECT_PLAYER_UP:
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_10, 3, TimeStart, true);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_10, 1, TimeStart+500, true);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_100, 3, TimeStart+600, true);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_1000, 3, TimeStart+900, true);
      break;
    case SOUND_EFFECT_GAME_START:
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_EXTRA, 4, TimeStart + 500);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_10, 3, TimeStart + 679);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_100, 3, TimeStart + 846);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_1000, 3, TimeStart + 1013);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_1000, 3, TimeStart + 1208);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_1000, 3, TimeStart + 1313);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_1000, 3, TimeStart + 1408);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_100, 3, TimeStart + 1546);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_100, 3, TimeStart + 1746);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_100, 3, TimeStart + 1842);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_100, 3, TimeStart + 1942);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_10, 3, TimeStart + 2079);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_100, 3, TimeStart + 2246);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_10, 3, TimeStart + 2413);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_EXTRA, 3, TimeStart + 2579);
      break;
    case SOUND_EFFECT_EXTRA_BALL:
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_EXTRA, 3, TimeStart, true);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_10, 3, TimeStart + 100, true);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_100, 3, TimeStart + 200, true);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_1000, 3, TimeStart + 300, true);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_100, 3, TimeStart + 500, true);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_1000, 3, TimeStart + 600, true);
      break;
    case SOUND_EFFECT_5K_CHIME:
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_EXTRA, 3, TimeStart +  0);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_1000, 3, TimeStart +  50);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_10, 3, TimeStart +   150);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_100, 3, TimeStart +  200);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_10, 3, TimeStart +   300);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_100, 3, TimeStart +  350);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_10, 3, TimeStart +   450);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_100, 3, TimeStart +  500);
      break;
    case SOUND_EFFECT_BALL_CAPTURE:
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_10, 3, TimeStart + 0);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_100, 3, TimeStart + 100);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_1000, 3, TimeStart + 200);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_100, 3, TimeStart + 300);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_10, 3, TimeStart + 400);
    break;
    case SOUND_EFFECT_POP_MODE:
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_EXTRA, 3, TimeStart + 0);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_EXTRA, 3, TimeStart + 100);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_EXTRA, 3, TimeStart + 200);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_EXTRA, 3, TimeStart + 300);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_10, 3, TimeStart + 425);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_10, 3, TimeStart + 525);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_10, 3, TimeStart + 625);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_10, 3, TimeStart + 725);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_100, 3, TimeStart + 850);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_100, 3, TimeStart + 950);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_100, 3, TimeStart + 1050);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_100, 3, TimeStart + 1150);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_1000, 3, TimeStart + 1275);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_1000, 3, TimeStart + 1375);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_1000, 3, TimeStart + 1475);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_1000, 3, TimeStart + 1575);
    break;
    case SOUND_EFFECT_POP_100:
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_100, 3, TimeStart + 0);
    break;
    case SOUND_EFFECT_POP_1000:
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_1000, 3, TimeStart + 10);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_EXTRA, 3, TimeStart + 20);
    break;
    case SOUND_EFFECT_POP_1000b:
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_10, 3, TimeStart + 10);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_EXTRA, 3, TimeStart + 20);
    break;
    case SOUND_EFFECT_POP_1000c:
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_100, 3, TimeStart + 10);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_EXTRA, 3, TimeStart + 20);
    break;
    case SOUND_EFFECT_TILT_WARNING:
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_1000, 3, TimeStart + 0);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_EXTRA, 3, TimeStart + 200);
    break;
    case SOUND_EFFECT_TILTED:
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_1000, 3, TimeStart + 0, true);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_EXTRA, 3, TimeStart + 400, true);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_1000, 3, TimeStart + 800, true);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_EXTRA, 3, TimeStart + 1200, true);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_1000, 3, TimeStart + 1600, true);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_EXTRA, 3, TimeStart + 2000, true);
    break;
    ...
    case SOUND_EFFECT_SPINNER_COMBO:
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_1000, 3, TimeStart + 0);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_100, 3, TimeStart + 75);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_10, 3, TimeStart + 135);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_1000, 3, TimeStart + 435);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_100, 3, TimeStart + 510);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_10, 3, TimeStart + 570);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_1000, 3, TimeStart + 870);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_100, 3, TimeStart + 945);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_10, 3, TimeStart + 1005);
      BSOS_PushToTimedSolenoidStack(SOL_KNOCKER, 3, TimeStart + 1105);
    break;
    case SOUND_EFFECT_KICKER_OUTLANE:
      ...
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_EXTRA, 3, TimeStart + 200);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_EXTRA, 3, TimeStart + 300);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_EXTRA, 3, TimeStart + 400);
    break;
    case SOUND_EFFECT_8_BALL_CAPTURE:
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_1000, 3, TimeStart + 0);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_100, 3, TimeStart + 100);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_10, 3, TimeStart + 200);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_EXTRA, 3, TimeStart + 300);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_10, 3, TimeStart + 400);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_EXTRA, 3, TimeStart + 500);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_10, 3, TimeStart + 600);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_100, 3, TimeStart + 700);
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_1000, 3, TimeStart + 800);
      BSOS_PushToTimedSolenoidStack(SOL_KNOCKER, 3, CurrentTime + 900);
    break;
  ...
  if (GameMode[CurrentPlayer] == 1) {  // Mode 1
    for(int i=0; i<7; i++) {                    // Cycle through 7 balls to set the lamps
      if (((Balls[CurrentPlayer] & (0b1<<(i + (CurrentPlayer%2 ? 8:0))))>>(i + (CurrentPlayer%2 ? 8:0))) == true) {
        BSOS_SetLampState((LA_BIG_1 + i + (CurrentPlayer%2 ? 8:0)), 1);       // if true turn large center lamp on
      } else {
        BSOS_SetLampState((LA_SMALL_1 + i + (CurrentPlayer%2 ? 8:0)), 1);     // if large lamp is off, small lamp is on
      }
    }
  ...
  if (GameMode[CurrentPlayer] == 2) {
    for(int i=0; i<15; i++) {                      // Cycle through 15 balls to set the lamps
      ...
      if ( ((Balls[CurrentPlayer] & (0b1<<i)) >> i ) == true) {
        BSOS_SetLampState((LA_BIG_1 + i), 1);      // if true turn large center lamp on
      } else {
        BSOS_SetLampState((LA_SMALL_1 + i), 1);    // if large lamp is off, small lamp is on
      }
    }
    // Turn on Ball 8 as special case, check first if Ball 8 is captured, if not check if 7 balls captured.
    if ((Balls[CurrentPlayer] & (0b10000000)) == 128) {
...
void CheckHighScores() {
  ...
  if (highestScore > HighScore) {
    ...
    BSOS_PushToTimedSolenoidStack(SOL_KNOCKER, 3, CurrentTime, true);
    BSOS_PushToTimedSolenoidStack(SOL_KNOCKER, 3, CurrentTime + 250, true);
...
// Default speeds, Segment 1 = 100
  ...
  if ((Segment == 1) || (Segment == 4)) {
    // Rotate CW
    // First leg
    BSOS_SetLampState(LA_BIG_1, (CW == true)?(MQPhase1==0):(MQPhase1==3), 0); 
    BSOS_SetLampState(LA_BIG_2, (CW == true)?(MQPhase1==1):(MQPhase1==2), 0);
    BSOS_SetLampState(LA_BIG_3, (CW == true)?(MQPhase1==2):(MQPhase1==1), 0);
    BSOS_SetLampState(LA_BIG_4, (CW == true)?(MQPhase1==3):(MQPhase1==0), 0);
    // Second leg
    BSOS_SetLampState(LA_BIG_5, (CW == true)?(MQPhase1==0):(MQPhase1==3), 0); 
    BSOS_SetLampState(LA_BIG_9, (CW == true)?(MQPhase1==1):(MQPhase1==2), 0);
    BSOS_SetLampState(LA_BIG_12, (CW == true)?(MQPhase1==2):(MQPhase1==1), 0);
    BSOS_SetLampState(LA_BIG_14, (CW == true)?(MQPhase1==3):(MQPhase1==0), 0);
    // Third leg
    BSOS_SetLampState(LA_BIG_15, (CW == true)?(MQPhase1==0):(MQPhase1==3), 0); 
    BSOS_SetLampState(LA_BIG_13, (CW == true)?(MQPhase1==1):(MQPhase1==2), 0);
    BSOS_SetLampState(LA_BIG_10, (CW == true)?(MQPhase1==2):(MQPhase1==1), 0);
    BSOS_SetLampState(LA_BIG_6, (CW == true)?(MQPhase1==3):(MQPhase1==0), 0);
    // Inner rack lamps 
    BSOS_SetLampState(LA_BIG_7, (CW == true)?(MQPhase2==0):(MQPhase2==2), 0); 
    BSOS_SetLampState(LA_BIG_11, (CW == true)?(MQPhase2==1):(MQPhase2==1), 0);
    BSOS_SetLampState(LA_BIG_8, (CW == true)?(MQPhase2==2):(MQPhase2==0), 0);
  }

  byte MQPhase3 = (CurrentTime/(Speed*54/100))%6; //  6 steps
  if ((Segment == 2) || (Segment == 4)) {
    // Bank Shot lamps
    BSOS_SetLampState(LA_BANK_SHOT_5000, ((CW == true)?(MQPhase3==0):(MQPhase3==5))||((CW == true)?(MQPhase3==1):(MQPhase3==0)), ((CW == true)?(MQPhase3==1):(MQPhase3==0))); 
    BSOS_SetLampState(LA_BANK_SHOT_1500, ((CW == true)?(MQPhase3==1):(MQPhase3==4))||((CW == true)?(MQPhase3==2):(MQPhase3==5)), ((CW == true)?(MQPhase3==2):(MQPhase3==5)));
    BSOS_SetLampState(LA_BANK_SHOT_1200, ((CW == true)?(MQPhase3==2):(MQPhase3==3))||((CW == true)?(MQPhase3==3):(MQPhase3==4)), ((CW == true)?(MQPhase3==3):(MQPhase3==4)));
    BSOS_SetLampState(LA_BANK_SHOT_900, ((CW == true)?(MQPhase3==3):(MQPhase3==2))||((CW == true)?(MQPhase3==4):(MQPhase3==3)), ((CW == true)?(MQPhase3==4):(MQPhase3==3)));
    BSOS_SetLampState(LA_BANK_SHOT_600, ((CW == true)?(MQPhase3==4):(MQPhase3==1))||((CW == true)?(MQPhase3==5):(MQPhase3==2)), ((CW == true)?(MQPhase3==5):(MQPhase3==2)));
    BSOS_SetLampState(LA_BANK_SHOT_300, ((CW == true)?(MQPhase3==5):(MQPhase3==0))||((CW == true)?(MQPhase3==0):(MQPhase3==1)), ((CW == true)?(MQPhase3==0):(MQPhase3==1)));
  }
  ...
  if ((Segment == 3) || (Segment == 4)) {
    // Multiplier lamps
    BSOS_SetLampState(LA_5X, MQPhase4==0||MQPhase4==1, 0); 
    BSOS_SetLampState(LA_3X, MQPhase4==1||MQPhase4==2, 0);
    BSOS_SetLampState(LA_2X, MQPhase4==2||MQPhase4==0, 0);
  
    // Upper alley Marquee
    // Top Row
    BSOS_SetLampState(LA_SMALL_1, MQPhase5==0||MQPhase5==1||MQPhase5==2, MQPhase5==2); 
    BSOS_SetLampState(LA_SMALL_2, MQPhase5==1||MQPhase5==2||MQPhase5==3, MQPhase5==3);
    BSOS_SetLampState(LA_SMALL_3, MQPhase5==2||MQPhase5==3||MQPhase5==4, MQPhase5==4);
    BSOS_SetLampState(LA_SMALL_4, MQPhase5==3||MQPhase5==4||MQPhase5==5, MQPhase5==5);
    // Bottom Row
    BSOS_SetLampState(LA_SMALL_12, MQPhase5==4||MQPhase5==5||MQPhase5==6, MQPhase5==6); 
    BSOS_SetLampState(LA_SMALL_11, MQPhase5==5||MQPhase5==6||MQPhase5==7, MQPhase5==7);
    BSOS_SetLampState(LA_SMALL_10, MQPhase5==6||MQPhase5==7||MQPhase5==0, MQPhase5==0);
    BSOS_SetLampState(LA_SMALL_9, MQPhase5==7||MQPhase5==0||MQPhase5==1, MQPhase5==1);
  }

  byte MQPhase6 = (CurrentTime/Speed)%4; // 4 steps
  if (Segment == 4) {
    // Player Lamps
    BSOS_SetLampState(LA_1_PLAYER, MQPhase6==0||MQPhase6==1, 0); 
    BSOS_SetLampState(LA_2_PLAYER, MQPhase6==1||MQPhase6==2, 0);
    BSOS_SetLampState(LA_3_PLAYER, MQPhase6==2||MQPhase6==3, 0);
    BSOS_SetLampState(LA_4_PLAYER, MQPhase6==3||MQPhase6==0, 0);

    // Remaining lamps
    BSOS_SetLampState(LA_SMALL_5, 1, 0, 125);
    BSOS_SetLampState(LA_SMALL_13, 1, 0, 250);
    BSOS_SetLampState(LA_SMALL_6, 1, 0, 125);
    BSOS_SetLampState(LA_SMALL_14, 1, 0, 250);
    BSOS_SetLampState(LA_SMALL_7, 1, 0, 125);
    BSOS_SetLampState(LA_SMALL_15, 1, 0, 250);
  
    BSOS_SetLampState(LA_SMALL_8, 1, 0, 250);
    BSOS_SetLampState(SAME_PLAYER, 1, 0, 500);
    BSOS_SetLampState(LA_SUPER_BONUS, 1, 0, 500);
    BSOS_SetLampState(LA_SPINNER, 1, 0, 250);
    BSOS_SetLampState(LA_OUTLANE_SPECIAL, 1, 0, 125);
...
//  }
  ...
  if ( (CurrentTime - ChimeScoringDelay) > (((Silent_Thousand_Pts_Stack) || (Silent_Hundred_Pts_Stack))?WaitTimeShort:WaitTimeLong) ) {
    ...
    if (Silent_Hundred_Pts_Stack > 0) {
      ...
      PlaySoundEffect(SOUND_EFFECT_10_PTS);
      //BSOS_PushToTimedSolenoidStack(SOL_CHIME_10, 3, CurrentTime + 0);
      CurrentScores[CurrentPlayer] += 10;
      Ten_Pts_Stack -= 1;
      ...
      //BSOS_PushToTimedSolenoidStack(SOL_CHIME_100, 3, CurrentTime + 0);
      CurrentScores[CurrentPlayer] += 100;
      Hundred_Pts_Stack -= 1;
      ...
      PlaySoundEffect(SOUND_EFFECT_1000_PTS);
      //BSOS_PushToTimedSolenoidStack(SOL_CHIME_1000, 3, CurrentTime + 0);
      CurrentScores[CurrentPlayer] += 1000;
...
//    if (CurrentTime > (MatchSequenceStartTime + MatchDelay)) {
      ...
      BSOS_PushToTimedSolenoidStack(SOL_CHIME_EXTRA, 1, 0, true);
      BIPDispValue = ((int)MatchDigit * 10);
